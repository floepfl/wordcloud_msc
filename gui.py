...
from wordcloud import transform
...
def main_window(font, image):
    ...
    text_size = (20, 1)
    button_size = (15, 1)
    layout = [[sg.Column([[sg.Text('Input image: ', font=font, size=text_size), sg.Input(key='_FILES_', default_text='.png or .jpg/jpeg', font=font), sg.FilesBrowse(font=font, size=(10,1))],
                [sg.Text('Words:', font=font, size=text_size), sg.Input(key='_FILES_1', default_text='.numbers, .xlsx or .csv', font=font), sg.FilesBrowse(font=font, size=(10,1))],
                ...
                [sg.Button('Reset to default settings', key='reset', size=button_size, font=font), sg.Button('Transform', key='transform', size=(15,2), font=font)]]),
               image]]
...
def loading():
    layout = [
        [sg.Image(r'C:\PySimpleGUI\Logos\loading_cat.gif', key='loading_gif')],
    ]
    return sg.Window('Window Title', layout)\
        .Element('_IMAGE_')\
        .UpdateAnimation(gif103, time_between_frames=50)